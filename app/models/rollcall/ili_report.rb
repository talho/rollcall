class Rollcall::ILIReport < ::Report
  ...
  def self.build_report(user_id)
    ...
    # Don't be dumb and make these three queries respect the user's schools or school districts
    params[:totals] = user.schools.joins(:school_daily_infos).select('report_date, SUM(total_enrolled) as enrolled, SUM(total_absent) as absent, SUM(total_absent)::float/nullif(SUM(total_enrolled), 0) as rate').reorder(:report_date)
                    .group(:report_date).having('report_date > (current_date - 7) AND report_date <= current_date').as_json
    confirmed = user.schools.joins(:students => [:student_daily_info]).select("report_date, count(*) as total").where("confirmed_illness = true").reorder(:report_date).group(:report_date).where("report_date > (current_date -7) AND report_date <= current_date").as_json
    ili = user.schools.joins(:students => {:student_daily_info => :symptoms}).select("report_date, count(*) as total").where("icd9_code in (#{icd9_code_string})").where("report_date > (current_date -7) AND report_date <= current_date").reorder(:report_date).group(:report_date).as_json
    ...
    school_districts.each do |sd|
      ...
      # find ili
      val[:ili] = sd.student_daily_infos.select("report_date, count(*) as total").joins(:symptoms).where("icd9_code in (#{icd9_code_string})").where("report_date > (current_date -7) AND report_date <= current_date").order(:report_date).group(:report_date).as_json
