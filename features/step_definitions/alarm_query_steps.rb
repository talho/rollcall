...
Given /^I have an alarm query$/ do
  aq = Rollcall::AlarmQuery.new(:deviation => 1, :name => "Example Query", :start_date => DateTime.now, :school_ids => [Rollcall::School.find_by_name("Anderson Elementary").id])
  aq.save
  step %Q{I navigate to "Apps > Rollcall > Alarms"}
  step %Q{I wait for the panel to load}
end
