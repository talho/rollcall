...
Talho.Rollcall.NurseAssistant = Ext.extend(function(){}, {
  constructor: function(config)
  {
    ...
    this.main_panel_store = new Ext.data.GroupingStore({
      ...
      restful:        true,
      baseParams:     {filter_report_date: new Date(),school_id: this.user_school_id},
      listeners: {
        scope: this,
        load:  function(this_store, records){
          if(records.length != 0){
    ...
    this.main_panel = new Ext.Panel({
      ...
      items:    [{
        ...
        cm:       new Ext.grid.ColumnModel({
          columns: [
            {id:'student_number',    header:'Student Number',     dataIndex:'student_number', flex: 1},
            {id:'first_name_column', header:'Student First Name', dataIndex:'first_name', flex: 1},
            {id:'last_name_column',  header:'Student Last Name',  dataIndex:'last_name',  flex: 1},
            {id:'symptom_column',    header:'Symptoms',           dataIndex:'symptom', flex: 1},
            {id:'header',            header:'Action',             dataIndex:'treatment',  flex: 1},
            {id:'visit_date',        header:'Visit Date',         dataIndex:'report_date', flex: 1},
            {
              xtype:     'xactioncolumn',
        ...
        bbar: [
          ...
          new Ext.PagingToolbar({store: this.main_panel_store})
        ],
        tbar: {
          items: ['->',{
        ...
        items:    [{
          ...
          loadMask:   true,
          viewConfig: {
            emptyText: '<div style="color:#000;">No student selected.</div>',
            autoFill:  true
          },
          flex:       1,
  ...
  show_details: function(grid_panel, index, event)
  {
    if(!this.window_open){
      ...
      this.main_panel.getComponent('student_detail_panel').getComponent('history_grid').getStore().load({
        params: {id: this.details_template.record.get("id")}
      });
    }   
  },
