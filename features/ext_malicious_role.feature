Feature: Prevent Malicious Behavior
  In order to fully secure the system
  As a Rollcall user
  I should not be able to change or request roles through malicious JS attempts
...
Background:
  ...
  And the following users exist:
    | Nurse Betty  | nurse.betty@example.com  | Nurse          | Houston | rollcall |
    | Epi Smith    | epi.smith@example.com    | Epidemiologist | Houston | rollcall |
    | Health Offcr | heatlh.offcr@example.com | Health Officer | Houston | rollcall |
  ...
  And rollcall user "nurse.betty@example.com" has the following schools assigned:
    ...
    | Ashford Elementary  |
    | Yates High School   |
  And rollcall user "epi.smith@example.com" has the following school districts assigned:
    | Houston |
  And rollcall user "epi.smith@example.com" has the following schools assigned:
    | Anderson Elementary |
    | Ashford Elementary  |
    | Yates High School   |
  And rollcall user "heatlh.offcr@example.com" has the following school districts assigned:
    | Houston |
  And rollcall user "heatlh.offcr@example.com" has the following schools assigned:
    | Anderson Elementary |
    | Ashford Elementary  |
    | Yates High School   |
...
Scenario: Nurse tries to maliciously request Epidemiologist role
    When I am logged in as "nurse.betty@example.com"
    And I maliciously request the rollcall role "Epidemiologist" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests

Scenario: Nurse tries to maliciously request Health Officer role
    When I am logged in as "nurse.betty@example.com"
    And I maliciously request the rollcall role "Health Officer" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests

Scenario: Nurse tries to maliciously request Rollcall Admin role
    When I am logged in as "nurse.betty@example.com"
    And I maliciously request the rollcall role "Admin" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests

Scenario: Epidemiologist tries to maliciously request Nurse role
    When I am logged in as "epi.smith@example.com"
    And I maliciously request the rollcall role "Nurse" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests

Scenario: Epidemiologist tries to maliciously request Health Officer role
    When I am logged in as "epi.smith@example.com"
    And I maliciously request the rollcall role "Health Officer" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests

Scenario: Epidemiologist tries to maliciously request Rollcall Admin role
    When I am logged in as "epi.smith@example.com"
    And I maliciously request the rollcall role "Admin" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests

Scenario: Health Officer tries to maliciously request Nurse role
    When I am logged in as "heatlh.offcr@example.com"
    And I maliciously request the rollcall role "Nurse" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests

Scenario: Health Officer tries to maliciously request Epidemiologist role
    When I am logged in as "heatlh.offcr@example.com"
    And I maliciously request the rollcall role "Health Officer" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests

Scenario: Health Officer tries to maliciously request Rollcall Admin role
    When I am logged in as "heatlh.offcr@example.com"
    And I maliciously request the rollcall role "Admin" in "Texas"
    And I wait for 1 second
    Then I should have no pending role requests