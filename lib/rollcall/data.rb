  ...
  module Data
    def export_data params, filename, user_obj, obj
      update_array = obj.map do |row|
        array = row.build_graph_query(params, row).all
      end
    ...
    def build_graph_query query, params    
      ...
      results = apply_date_filter query, params[:startdt], params[:enddt]
      results = apply_data_function query, params[:data_func]
    ...
    def self.apply_ili_filters query, conditions       
      ...
      if conditions[:confirmed_illness] == "Confirmed Illness"
