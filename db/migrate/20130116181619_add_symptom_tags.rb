class AddSymptomTags < ActiveRecord::Migration
  def up
    # create symptom tagging table
    create_table :rollcall_symptom_tags do |t|
      t.string :match
    ...
    # modify ili import function to call symptom search function
    execute %Q{
      CREATE OR REPLACE FUNCTION rollcall_insert_student_info(_cid varchar(255), _year varchar(20), _tea_id int, _date date, _onset date, _temp float, _grade int, _zip int, _gender varchar(1), _doc varchar(200), _doc_address varchar(200),
        ...
        BEGIN
          ...
          IF _info_id IS NOT NULL THEN
            SELECT rollcall_symptom_match(_info_id);
          END IF;
        END;
      $$ LANGUAGE plpgsql;
    }
  ...
  def down
    # replace ILI import function with previous version
    execute %Q{
      CREATE OR REPLACE FUNCTION rollcall_insert_student_info(_cid varchar(255), _year varchar(20), _tea_id int, _date date, _onset date, _temp float, _grade int, _zip int, _gender varchar(1), _doc varchar(200), _doc_address varchar(200),
                                                              _symptoms text, _treatment text, _follow_up date, _confirmed boolean, _in_school boolean, _released boolean) RETURNS VOID AS $$
        DECLARE
    ...
    # drop symptom search function
    execute "DROP FUNCTION rollcall_symptom_match(int)"

    # drop symptom tagging table
    drop_table :rollcall_symptom_tags
  end
end
